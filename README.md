...
Every level of the order format hierarchy must have the "title" attribute set to a string that can be displayed by the ordering device and the "id" string set to an id that the device can use. These IDs should be unique, or undefined behavior may be reached. 
...
* The Type Menu Item
    ...
    * ChooseOne items are lists of items of which only one can be selected. This is useful if, for example, a car type must be selected. These items are specified by the "ChooseOne" key under a submenu. The value of this key must be JSON data that specifies a list of items to choose from with the "ChooseFrom" key. This item also must have a title and ID
    * ChooseMultiple items are lists of items of which as many as desired may be selected. This is useful as a list of ingredients for example. The "ChooseMultiple" key under a submenu must be set to a JSON object. In this object, the "ChooseFrom" key must be specified as a list of items to choose from. In addition, the "ExcludeFromEverything" key may be set to a list of items to not add when the user selects the "Everything" option. In addition, the "title" and "id" keys must also be set. 
    * LightHeavyExtra items are items that may be classified as none, light, normal, or extra. This is useful if there is a list of ingredients that can be present in different amounts. This may be placed under a submenu with the "LightHeavyExtra" key set to a JSON object. This object must have the title and id keys set. In addition, it must, like the ChooseMultiple item, have the "ChooseFrom" option set to a list of ingredients and may have the "ExcludeFromEverything" option set to a sublist of ChooseFrom.
    * Categories items are effectively submenus of their own. They are presented like ChooseOne Options, except they add different options depending on what is selected. 
